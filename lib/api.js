...
const API_URL = 'https://todo-be-n9y2.onrender.com/api' || 'http://localhost:8000/api';
...
export const fetchTodos = async () => {
  try {
    ...
    if(error && error.response && error.response.data && error.response.data.detail && error.response.data.detail == "Could not validate credentials"){
      Router.push('/login');
    }
    console.error('Error fetching todos:', error);
  }
